...
### O CÓDIGO
O arquivo principal está localizado dentro da pasta `src` e se chama `recintos-zoo.js`. Você pode desenvolver a sua lógica criando outros arquivos, métodos e até mesmo outras classes, porém o resultado deve poder ser obtido através do método `analisaRecintos`.
...
### VALIDANDO A SOLUÇÃO
Há alguns cenários de testes no arquivo `recintos-zoo.test.js` para auxiliar na validação da solução. Recomendo que você crie mais casos de teste para aumentar a confiabilidade da sua solução.
...
#### CÓDIGO
A solução está em **javascript** e a **estrutura de pastas e arquivos** está seguindo o indicado na seção `O CÓDIGO`.
